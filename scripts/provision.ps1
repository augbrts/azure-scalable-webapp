...
Write-Host ">> 'materiais' (az storage blob upload-batch ...)"
...
Write-Host ">> infra/cloud-init.yaml. FQDN do MySQL: $FQDN"
$OK = Read-Host "Cloud-init preenchido? [s/N]"
if ($OK -eq "s" -or $OK -eq "S") {
  ...
  Write-Host ">> VMSS adiada. Rode novamente apos preencher o cloud-init."
}
